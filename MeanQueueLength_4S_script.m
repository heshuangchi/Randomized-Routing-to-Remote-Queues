...
mu = 1.0; % service rate
lambda = 4*mu*rho; % four stations
delay = 100.0;
Chi = 0:0.001:0.5;

fprintf('Program starts at %s\n', datetime);
CompTimeStart = tic;
totalQL = mTotalQL_4SfDL(lambda, mu, delay, Chi, TimeHorizon, SampleSize);
fprintf('Program takes %.1f seconds\n', toc(CompTimeStart));

save(sprintf('QLvsChi4S_SampleSize%.1e_TimeHorizon%.1e_rho%.2f_delay%.1f.mat',...
