...
TimeHorizon = 1e5; % time horizon
SampleSize = 20; % total number of replications

rho = 0.99; % traffic intensity
mu = 1.0; % service rate
lambda = 2*mu*rho; % two stations
delay = 100.0;
Prob_JSQ = 0.50:0.1:1.0;

fprintf('Program starts at %s\n', datetime);
CompTimeStart = tic;
totalQL = mTotalQL_2SrDL(lambda, mu, delay, Prob_JSQ, TimeHorizon, SampleSize);
fprintf('Program takes %.1f seconds\n', toc(CompTimeStart));

% save(sprintf('SampleSize%d_TimeHorizon%.1e_rho%.2f_delay%d_ProbJSQ.mat', SampleSize, TimeHorizon, rho, delay));
