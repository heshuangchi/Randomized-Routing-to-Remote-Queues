...
rho = 0.9; % traffic intensity
mu = 1.0; % service rate
lambda = 5*mu*rho; % five stations
delay = 1.0;
Chi = 0:0.001:0.8;

fprintf('Program starts at %s\n', datetime);
CompTimeStart = tic;
totalQL = mTotalQL_5SfDL(lambda, mu, delay, Chi, TimeHorizon, SampleSize);
fprintf('Program takes %.1f seconds\n', toc(CompTimeStart));

save(sprintf('QLvsChi5S_SampleSize%.1e_TimeHorizon%.1e_rho%.2f_delay%.1f.mat',...
